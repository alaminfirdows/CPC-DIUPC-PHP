...
INSERT INTO `categorys` (`url`, `name`, `status`) VALUES ('uncategorised', 'Uncategorised', 1);
...
CREATE TABLE IF NOT EXISTS `events` (
  ...
  KEY `FK_categorys` (`category`),
  CONSTRAINT `FK_users` FOREIGN KEY (`authorId`) REFERENCES `users` (`id`) ON UPDATE CASCADE,
  CONSTRAINT `FK_categorys` FOREIGN KEY (`category`) REFERENCES `categorys` (`id`) ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `semester_activity` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `title` varchar(255) NOT NULL,
  `date` varchar(25),
  `time` varchar(25),
  ...
  `updatedAt` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `FK_semester_activity_semesters` (`semester`),
  KEY `FK_semester_activity_categorys` (`category`),
  CONSTRAINT `FK_semester_activity_categorys` FOREIGN KEY (`category`) REFERENCES `categorys` (`id`) ON UPDATE CASCADE,
  CONSTRAINT `FK_semester_activity_semesters` FOREIGN KEY (`semester`) REFERENCES `semesters` (`id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;